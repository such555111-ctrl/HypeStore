...
log = logging.getLogger("hypestore")
...
def cmd_start(message: types.Message):
    if not BASE_URL:
        bot.send_message(
            message.chat.id,
            "HypeStore: мини-апп ещё не сконфигурирован (не задан WEBHOOK_URL)."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "⚜️ *HypeStore*\n\n"
        "Добро пожаловать в HypeStore! Эксклюзивные дропы, кроссовки, худи и "
        "аксессуары — доставка по всему миру.\n\n"
...
def customer_cancel_order(oid):
    ...
    for admin_id in ADMIN_CHAT_IDS:
        try:
            bot.send_message(admin_id, f"❌ Клиент отменил заказ HS-{oid}")
        except Exception as e:
            log.warning("Не удалось уведомить администратора %s: %s", admin_id, e)
...
def admin_message_customer(oid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Сообщение от HypeStore по заказу HS-{oid}:\n\n{text}"
        )
    except Exception as e:
...
def admin_message_question(qid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Ответ от HypeStore по товару «{q.get('product_name','')}»:\n\n{text}"
        )
    except Exception as e:
